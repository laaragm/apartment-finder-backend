	...
	public async Task Execute(IJobExecutionContext context)
	{
		...
		foreach (var outboxMessage in outboxMessages)
		{
			...
			try
			{
				...
				await _publisher.Publish(domainEvent, context.CancellationToken); // Publish domain event and trigger respective event handler
			}
	...
	private async Task UpdateOutboxMessageAsync(IDbConnection connection, IDbTransaction transaction, OutboxMessageResponse outboxMessage, Exception? exception)
	{
		const string sql = @"
            ...
            SET processed_on_utc = @ProcessedOnUtc,
                error = @Error
            WHERE id = @Id";
		await connection.ExecuteAsync(sql, new { outboxMessage.Id, ProcessedOnUtc = _dateTimeProvider.UtcNow, Error = exception?.ToString() }, transaction: transaction);
	}
}
